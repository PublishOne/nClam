    ...
    public class ClamClient : IClamClient
    {
        ...
        private async Task<string> ExecuteClamCommandAsync(string command, CancellationToken cancellationToken, Func<NetworkStream, CancellationToken, Task> additionalCommand = null)
        {
            ...
            using (var clam = new TcpClient(AddressFamily.InterNetwork))
            {
                ...
                {
                    result = $"Error in ExecuteClamCommandAsync: NativeError={ex.NativeErrorCode}, SocketError={ex.SocketErrorCode}, Msg={ex.Message} at {ex.Source} ERROR";
                }
                catch (Exception ex)
                {
                    result = $"Error in ExecuteClamCommandAsync: {ex.Message} at {ex.Source} ERROR";
                }
                finally
                {
                    if (clam.Connected)
